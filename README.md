This project was created with Create React App; the super long instructions are below. But you probably don't care about those.

The main things: this is meant to pair with [Music Machine Learning Backend](https://github.com/coderschool/music_rex). 

To run this project, just clone it and then run:
* `npm install`
* `npm run start`

Then you should be able to open a port to localhost:3000, and run the project as normal.

The `music_rex` repo referenced above has a built version of this manually copied into the directory (which is built by doing `npm run build`). That'll generate a static directory, which has some relevant files. 
